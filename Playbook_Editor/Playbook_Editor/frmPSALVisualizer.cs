...
namespace Playbook_Editor
{
    public partial class frmPSALVisualizer : Form
    {
        ...
        private void mnuPSALVis_MouseDown(object sender, MouseEventArgs e)
        {
            mouseLocation = new Point(-e.X, -e.Y);
        }

        private void mnuPSALVis_MouseMove(object sender, MouseEventArgs e)
        {
        ...
        private void mnuPSALVis_DoubleClick(object sender, EventArgs e)
        {
            if (WindowState == FormWindowState.Maximized)
            {
                WindowState = FormWindowState.Normal;
        ...
        private void drawPSAL(bool SavePSAL)
        {
            ...
            currentPSAL.routeColor = Color.Black;
            ...
            if (psal.steps.Count > 0)
            {
                ...
                for (int i = 0; i < psal.steps.Count; i++)
                {
                    ...
                    if (DrawPSAL == true)
                    {
                        ...
                        //Convert PSAL
                        Program.convertPSAL(currentPSAL, LOS);

                        //draw PSAL
                        Program.drawRoute(g, currentPSAL);

                        //draw player icon
                        Program.drawPlayerCircle(g, Color.White, currentPSAL, 12);
                        ...
                        if (SavePSAL == true)
                        {
                            ...
                            gImage.Save(saveFolder + currentPSAL.ID.ToString() + "_ARTL_" + currentPSAL.ARTL.ToString() + ".jpg", ImageFormat.Jpeg);
                        }
        ...
        private void picPSAL_MouseDown(object sender, MouseEventArgs e)
        {
            if (chbMovePlayer.CheckState == CheckState.Checked)
            {
                ...
                psal.playerXY = new XY { X = mouseEventArgs.X, Y = mouseEventArgs.Y };
                drawPSAL(false);
            }
            if (e.Button == MouseButtons.Right)
            {
